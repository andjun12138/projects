...
function initPage() {
    $table.bootstrapTable({
        height: getHeight(),
        toolbar: '#toolbar',
        url: '/enterprise/permissionsQuery',
        method: 'post',
...
function responseHandler(res) {
    ...
    for(var i = 0;i < length;i++) {
        //成果名称
        data[i].name = '<a target="_blank" href="/enterprise/to_create_update/'+data[i].id+'.html">'+data[i].name+'</a>';
        data[i].iCustoms = data[i].iCustoms > 0 ? '是':'否';
        data[i].hatchery = data[i].hatchery > 0 ? '是':'否';
        data[i].cnkiCount = '<a href="javascript:void(0);" onclick="cnkiCountOnClick(\''+data[i].id+'\')">'+data[i].cnkiCount+' </a>';
...
function cnkiCountOnClick(enterpriseId) {
    layer.open({
        type: 2,
        title: '新建按钮',
        shadeClose: false,
        shade: 0.8,
        area: ['800px', '90%'],
        content: '/thirdCNKIUser/cnkiAccountManage/'+enterpriseId+'.html'
    });
}
