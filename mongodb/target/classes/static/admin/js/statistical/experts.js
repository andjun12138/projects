...
function getAnalysisData() {
    ...
    $.ajax({
        url: '/admin/experts/getTrendAnalysis',
        type: 'post',
        dataType: 'json',
...
function getProportionData() {
    ...
    $.ajax({
        url: '/admin/experts/getPercentage',
        type: 'post',
        dataType: 'json',
        contentType: 'application/json;charset=UTF-8',
        data: JSON.stringify(json),
...
function setEcharts(flag,data) {
    ...
    if (flag) {
        ...
        if (field == '类型') {
            for(var i = 0;i<data.length;i++) {
                xAxis.push(data[i].expert_type);
                series.push({
                    name: data[i].expert_type,
                    value: data[i].counts
                });
            }
        } else if (field == '领域') {
                ...
                feature: {
                    ...
                    myTool: {
                        ...
                        onclick: function () {
                            ...
                            getChartOption('pie', options.xAxis[0].data, options.series[0].data, options.title[0].title);
                        }
                    },
                    dataView: {readOnly: false},
                    restore: {
