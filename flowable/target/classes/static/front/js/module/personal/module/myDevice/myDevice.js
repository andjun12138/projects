$(function () {
    ...
    function MyDevice() {
        ...
        /* 设备表格 */
        // 设备表格
        var oTable = new Table('my-device-list-table');
        // 父节点
        var oTableParent = oNowModel.find('#my-device-list-table');
        // 设备数据
        ...
        // 顺序表
        var aTableOrder = ['device', 'deviceModel', 'time', 'status', 'id'];
        // 设备名html
        var sDeviceTableNameHtml = '<a id="%id%" class="device-name-link" href="javaScript:void(0);">%name%</a>';
        // 活动span Html
        var sDeviceTableActiveSpanHtml = '<span class="active-span" data-type="delete" data-id="%id%">删除</span>';
        // var sDeviceTableActiveSpanHtml = '<span class="active-span" data-type="edit" data-id="%id%" style="margin-right: 8px;">修改</span>' +
        //     '<span class="active-span" data-type="delete" data-id="%id%">删除</span>';
        // 是否正在取消预约
        ...
        function extractDeviceListData(data) {
            ...
            data.forEach(function (item) {
                ...
                obj.id = item.id;
                obj.device = {};
                obj.device.id = item.id;
                obj.device.name = item.name;
                obj.status = (!!item.backCheckStatus) ? JSON.parse(item.backCheckStatus).title : null;
                obj.deviceModel = item.model;
                obj.time = (!!item.backCheckTime) ? formatTime(new Date(item.backCheckTime), false, 'YYYY-MM-DD') : null;
                result.push(obj);
            });
            return result;
        }
    ...
    // 函数继承
    MyDevice.prototype = new FuncOfModelCommon();
    // 引用释放
    FuncOfModelCommon = null;
    // 调用主模块的模块注册方法
    personalMainObj.setMethodObj('myDevice', new MyDevice());
});
