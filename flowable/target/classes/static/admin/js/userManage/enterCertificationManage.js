...
function responseHandler(res) {
    ...
    for(var i = 0;i < length;i++) {
        data[i].name = '<a target="_blank" href="/enterCertification/to_create_update/'+data[i].id+'.html">'+data[i].name+'</a>';
        //    审核状态
        if(data[i].status != null){
            data[i].status = JSON.parse(data[i].status).title;
        }
        if(data[i].belongIndustry != null){
            data[i].belongIndustry = JSON.parse(data[i].belongIndustry).title;
        }
    }
    return {
