...
function responseHandler(res) {
    ...
    for (var i = 0; i < length; i++) {
        //处理模块分组
        var moduleStr = res.data.data_list[i].module_id;
        var moduleObj = JSON.parse(moduleStr);
        res.data.data_list[i].module_id = moduleObj.title;

        var searchEngineStr = res.data.data_list[i].save_to_search_engine;
        if(searchEngineStr == true){
            res.data.data_list[i].save_to_search_engine = '是';
        }else {
            res.data.data_list[i].save_to_search_engine = '否';
        }

        var appIndexTypeStr = res.data.data_list[i].app_index_type;
        if(appIndexTypeStr == -1){
            res.data.data_list[i].app_index_type = '管理库';
        }else if(appIndexTypeStr == -2){
            res.data.data_list[i].app_index_type = '开发库';
        }else if(appIndexTypeStr == 0){
            res.data.data_list[i].app_index_type = '学校库';
        }
    }
    return {
...
function initSearchOptions() {
    ...
    searchOptions['moduleId'] = $("#searchModule").val();
    searchOptions['isSearch'] = $("#isSearch").val();
    searchOptions['appIndexType'] = $("#appIndexType").val();
}
//表格数据获取的参数
function queryParams(params) {
    ...
    if (StringNoEmpty(searchOptions.moduleId)) postData['moduleId'] = searchOptions.moduleId;
    if (StringNoEmpty(searchOptions.isSearch)) postData['saveToSearchEngine'] = searchOptions.isSearch;
    if (StringNoEmpty(searchOptions.appIndexType)) postData['appIndexType'] = searchOptions.appIndexType;
