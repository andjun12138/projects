// 保存或修改接口  /admin/shufflingFigure/create_update  POST
/*
 * create by mujun at 2018/8/21
 */
var $save = $('#save'),
    $close = $("#close"),
    $reset = $('#reset'),
...
function initPage() {
    if(!!qualificationId){
    //    修改页
        var icon = $('#icon').data('pic');
        var picList = $('.file-list:eq(0)');
        var filename=icon[0].title+"."+icon[0].prefix;
        var iconSrc='/static/assets/photo.png';
        picList.append('<div class="file-item"><img class="file-icon pull-left" src="' + iconSrc + '"/><div class="file-name" title="' + filename + '" data-id="'+icon[0].id+'">' + filename + '</div><i class="icon-remove-sign pull-right file-remove" title="删除"></i></div>');
    ...
    $save.on('click', function (e) {
        ...
        if (!!$('.file-name').data('id')) {
            postData['icon'] = $('.file-name').data('id');
        } else {
            layer.msg('请上传至少一张图片',function(){});
            return ;
        }
        if (!!qualificationId) {
            postData['id'] = qualificationId;
            $.ajax({
                url: '/admin/qualification/create_update',
