...
function addUser(){
    ...
    var postData = {
        ...
        password: passwordEncrypt($('#user-password').val()) || null,
        mobile: $('#user-mobile').val() || null,
        email: $('#user-email').val() || null
    };
    $.ajax({
        type:'post',
        url: '/systemUser/create_update',
...
function setUserRole() {
    ...
    if(selectedIds.length === 0) {
        ...
        var url = '/systemUser/' + selectedIds[0].id + '/roles';
        var title = '分配角色';
        layer.open({
            type: 2,
            content: url,
