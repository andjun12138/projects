// 保存或修改接口 /infrastructure/create_update  POST
/*
...
function initPage() {
    if(!!infrastructureId){
        ...
        console.log('1111111111111111111111111111',picture);
        if(!!picture){
            var picList = $('.file-list:eq(0)');
            // for (var i = 0; i < picture.length; i++){
                var filename=picture.title+"."+picture.prefix;
                var iconSrc='/static/assets/photo.png';
                picList.append('<div class="file-item"><img class="file-icon pull-left" src="' + iconSrc + '"/><div class="file-name" title="' + filename + '" data-id="'+picture.id+'">' + filename + '</div><i class="icon-remove-sign pull-right file-remove" title="删除"></i></div>');
                fileLength++ ;
                $("#logoInput").val("已选择"+fileLength+"张图片");
            // }
        }
    ...
    $save.on('click', function (e) {
        ...
        if (!!$('.file-name').data('id')) {
            var fileIds = "";
            var fileObjects = $('div.file-name');
            for(var i = 0; i < fileObjects.length; i++){
                // console.log('2222222222222',fileObjects[i]);
                fileIds += fileObjects.eq(i).attr('data-id');
                if(i != fileObjects.length-1){
                    fileIds += ",";
                }
            }
            postData['pictures'] = fileIds;
        } else {
            layer.msg('请上传至少一张图片',function(){});
            return ;
        }
        if (!!infrastructureId) {
            postData['id'] = infrastructureId;
            $.ajax({
                url: '/infrastructure/create_update',
    ...
    $(document).on('change','input[type="file"]', function (e) {
            ...
            if(xhr.status===200){
                ...
                for(var i = 0; i < list.length; i++){
                    target.append('<div class="file-item">' +
                        ...
                        '<div class="file-name" title="' + list[i].title + '" data-id="'+list[i].id+'">' + list[i].title+'.'+list[i].prefix+ '</div>' +
        ...
        layer.confirm('你确定删除该logo图片',{
            ...
            inputDom.val("已选择"+fileLength+"个文件");
            layer.closeAll();
        },function () {
            layer.closeAll();
